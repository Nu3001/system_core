...
#endif
static void ubcsp_put_stream_uart (uint8* stream, uint16 length)//Eshin 20140905 add
{
	if(length)
	{  	put_stream(stream, length);
     		PrintData(stream, length);
	}
}
/*****************************************************************************/
/**                                                                         **/
...
static void ubcsp_setup_packet (uint8 *part1, uint8 calc, uint8 *part2, uint16 len2)
{
	...
	//printf("\nubcsp_setup_packet:");	
	//PrintData(part1, calc);
	//PrintData(part2, len2);
	...
	*activity = 0;
	...
	if (!ubcsp_config.delay)
	{
		...
		if (ubcsp_config.send_size)
		{
			...
			{
				//Eshin 20140905 change start
				/* get the value to send, and calculate CRC as we go */

				/* get the stream start position(ptr) to send */
				uint8* startptr=ubcsp_config.send_ptr;
				uint16 i=0, length=0;

				for(i=0;i<ubcsp_config.send_size;i++)
				{
				value = *(ubcsp_config.send_ptr+i);
				#if UBCSP_CRC
				...
				if(value==SLIP_FRAME||value==SLIP_ESCAPE)
					{
					/* get the value to send */
					ubcsp_put_stream_uart(startptr,length);
					
					
					if(value==SLIP_FRAME)
						{/* get the SLIP data to send */
						put_uart (SLIP_ESCAPE);
						put_uart (SLIP_ESCAPE_FRAME);
						}
					else if(value==SLIP_ESCAPE)
						{/* get the SLIP data to send */
						put_uart (SLIP_ESCAPE);
						put_uart (SLIP_ESCAPE_ESCAPE);
						}
					/* reset the stream start position(ptr) to send */
					startptr=(ubcsp_config.send_ptr+i+1);
					length=0;

					
					}
				else
					{length++;/* get the stream length to send */
					}

				}
				/* get the rest data to send */
				ubcsp_put_stream_uart(startptr,length);
				/* keep code with orginal code structure */
				ubcsp_config.send_ptr=ubcsp_config.send_ptr+i;
				ubcsp_config.send_size=ubcsp_config.send_size-i;//=0;
				
			}

			/* If we did output a SLIP_ESCAPE, then don't process the end of a block */

			//Eshin for no use after rewrite code
			//Eshin 20140905 change End
			{
		...
		else if (ubcsp_config.link_establishment_packet == ubcsp_le_none)
		{
			...
			{
				/* Send the start of FRAME packet */
				printf("55Send the start of FRAME packet");
				put_uart (SLIP_FRAME);
