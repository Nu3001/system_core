    ...
    for(;;) {
        ...
        if(ttyS3_fd < 0) {
            if((ttyS3_fd = open("/dev/ttyS3", O_RDWR|O_NOCTTY|O_NONBLOCK)) < 0) {
                ERROR("==============bonovo open ttyS3 Mesg:%s\n",strerror(errno));
                ERROR("==============bonovo dev/ttyS3 open result = %d\n",ttyS3_fd);
            } else {
                ...
                tcflush(ttyS3_fd,TCIFLUSH);         /* Update the options and do it NOW */
                tcsetattr(ttyS3_fd,TCSANOW,&tios) ;
                ...
                //ERROR("==============bonovo dev/ttyS3 open result======== %d=======%d\n",ttyS3_fd,dwByteWrite);
